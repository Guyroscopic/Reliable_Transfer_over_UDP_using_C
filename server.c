		...
		//if valid file
			...
			for(i=0; i<totalSegements; i = i+windowSize){
					...
					if(totalSegements - i >= windowSize){
						...
						}//end for(int j=i; j<i+windowSize; j++)
						...
						while(k < windowSize){
							if((sent_sgmt[k]).ID != recievedIDs[k]){
								...
								while(ack_num != (sent_sgmt[k]).ID){
									//printf("Retransmitting Segement with ID: %d\n", sent_sgmt[k].ID);
									sendto(sock, &(sent_sgmt[k]),0,sizeof(sgmt),(struct sockaddr *)&client,clientlen);
									//printf("Segement Sent\n");
									recvfrom(sock,&(ack_num),sizeof(ack_num),0,(struct sockaddr *)&client,(socklen_t *)&clientlen);
									//printf("Recievd Ack\n");
								}//end while				
						
							}//end if((sent_sgmt[k]).ID != recievedIDs[k])
							else{
					...
					}//end if(totalSegements - i > windowSize)			
				...
				if(sgmt.length < bufferSize){
					...
					if(ferror(file)){
						printf("Encountered and Unexpected Error!\nProgram Terminated");
					}
					break;
				}	//end if(sgmt.length < bufferSize)					


			}//end for transmit data
		}//end of if(file != NULL)
	}//end of if((strcmp(buffer,"get") == 0))
